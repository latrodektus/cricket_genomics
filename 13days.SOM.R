...
AveCounts <- data.frame(Ave_LV_F_13,Ave_LV_M_13,Ave_No_F_13,Ave_No_M_13)
...
scales_list=list(x=list(labels=c("","LV_F_13","LV_M_13","No_F_13","No_M_13"),at=seq(0,4,1),cex=0.6))
...
labels=unique(som_data_pivot[,'cell'])
xyplot (y ~ x | cell,
data=som_data_pivot,
groups= grouping,
scales=scales_list,
...
user.defined.color=as.character(colours) ,user.defined.groups= grouping,
user.defined.scales= scales_list,
panel = function(x, y,user.defined.color,user.defined.groups,scales=scales,subscripts) {
	my_fill <- user.defined.color[subscripts]
	my_group <- user.defined.groups[subscripts]
	panel.xyplot(x, y, type=c("l", "p"),cex=0.3, col=my_fill, groups=my_group,subscripts=TRUE)
	panel.text(1,1.5,som_data_pivot[,'cell'][subscripts])
} )
cell_for_each_gene_id <- unique(som_data_pivot[,c("id_a", "id_b", "cell")])
write.table(cell_for_each_gene_id, "1days_cell_for_each_geneID_5x5.txt", sep="\t")